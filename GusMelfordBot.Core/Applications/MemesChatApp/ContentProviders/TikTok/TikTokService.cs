    ...
    public class TikTokService : ITikTokService
    {
        ...
        public async Task ProcessMessage(Message message)
        {
            ...
            try
            {
                ...
                {
                    int count = _databaseManager.Context.Set<TikTokVideoContent>().Count();

                    await EditMessageFromTelegram(
                        GetEditedMessage(tikTokVideoContent, count), 
                        message.Chat.Id,
                        messageId);
                   
                    tikTokVideoContent.MessageId = int.Parse(messageId);
                    tikTokVideoContent.AccompanyingCommentary = GetAccompanyingCommentaryIfExist(message.Text);
        ...
        {
            HttpResponseMessage httpResponseMessage =
                await _gusMelfordBotService.SendMessageAsync(new EditMessageParameters
                {
                    Text = newText,
